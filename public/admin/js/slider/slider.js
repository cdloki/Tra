let Slider = {
    ...
    save: function (is_quit = 0) {
        ...
        if (form.valid()) {
            ...
            data.push({ name: "description_slider", value: getValueTinymce("description_slider") });
            data.push({ name: "keyword_slider", value: getValueTinymce("keyword_slider") });

            $.ajax({
                url: laroute.route('sliders.store'),
                method: 'POST',
                data: data,
                success: function (res) {
                    if (res.error == false) {
                        Swal.fire({
                            ...
                            .then(function () {
                                is_quit == 0 ? window.location.href = laroute.route('sliders.index') : location.reload(true);
                            });
    ...
    changeStatus: function (o, id) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: laroute.route('sliders.change-status'),
                    method: 'POST',
                    data: {
                        id: id
                    },
    ...
    remove: function (o, id) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: laroute.route('sliders.remove'),
                    method: 'POST',
                    data: {
                        id: id
                    },
    ...
    update: function (is_quit = 0) {
        ...
        if (form.valid()) {
            ...
            $.ajax({
                url: laroute.route('sliders.update'),
                method: 'POST',
                data: data,
                success: function (res) {
                    if (res.error == false) {
