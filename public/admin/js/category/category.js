let Category = {
        ...
        form.validate({
            ...
            messages: {
                code_category: {
                    ...
                    maxlength: "Mã danh mục không được dài hơn 50 kí tự!",
                },
                name_category: {
                    required: "Tên danh mục không được để trống!",
                    maxlength: "Tên danh mục không được dài hơn 50 kí tự!",
                },
            },
        });
        if (form.valid()) {
            ...
            // let dataRoom = $('#id_room')[0].selectr.getValue()
            data.push({ name: "dataTagSeo", value: dataTagSeo });
            data.push({ name: "description_category", value: getValueTinymce("description_category") });
            data.push({ name: "description_seo", value: getValueTinymce("description_seo") });
            // data.push({ name: "dataRoom", value: dataRoom });
            $.ajax({
                url: laroute.route('categorys.store'),
                method: 'POST',
                data: data,
                success: function (res) {
                    if (res.error == false) {
                        Swal.fire({
                            ...
                            .then(function () {
                                is_quit == 0 ? window.location.href = laroute.route('categorys.index') : location.reload(true);
                            });
    ...
    changeStatus: function (o, id) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: laroute.route('categorys.change-status'),
                    method: 'POST',
                    data: {
                        id: id
                    },
    ...
    remove: function (o, id) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: laroute.route('categorys.remove'),
                    method: 'POST',
                    data: {
                        id: id
                    },
    ...
    update: function (is_quit = 0) {
        ...
        if (form.valid()) {
            ...
            $.ajax({
                url: laroute.route('categorys.update'),
                method: 'POST',
                data: data,
                success: function (res) {
                    if (res.error == false) {
