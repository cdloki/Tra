let Room = {
                ...
                success: function (res) {
                    if (res.error == false) {
                        Swal.fire({
                            ...
                            .then(function () {
                                is_quit == 0 ? window.location.href = laroute.route('rooms.index') : location.reload(true);
                            });
    ...
    changeStatus: function (o, id) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: laroute.route('rooms.change-status'),
                    method: 'POST',
                    data: {
                        id: id
                    },
    ...
    remove: function (o, id) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: laroute.route('rooms.remove'),
                    method: 'POST',
                    data: {
                        id: id
                    },
    ...
    update: function (is_quit = 0) {
        ...
        if (form.valid()) {
            ...
            data.push({ name: "description_seo", value: getValueTinymce("description_seo") });
            $.ajax({
                url: laroute.route('rooms.update'),
                method: 'POST',
                data: data,
                success: function (res) {
                    if (res.error == false) {
