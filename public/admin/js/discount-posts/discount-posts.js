let DiscountPost = {
        ...
        form.validate({
            ...
            messages: {
                name_post: {
                    ...
                    maxlength: "Tiêu đề bài viết không được dài hơn 100 kí tự!",
                },
            },
        });
        if (form.valid()) {
            ...
            data.push({ name: "content_post", value: getValueTinymce("content_post") });

            $.ajax({
                url: laroute.route('discount-posts.store'),
                method: 'POST',
                data: data,
                success: function (res) {
                    if (res.error == false) {
                        Swal.fire({
                            ...
                            .then(function () {
                                is_quit == 0 ? window.location.href = laroute.route('discount-posts.index') : location.reload(true);
                            });
    ...
    changeStatus: function (o, id) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: laroute.route('discount-posts.change-status'),
                    method: 'POST',
                    data: {
                        id: id
                    },
    ...
    remove: function (o, id) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: laroute.route('discount-posts.remove'),
                    method: 'POST',
                    data: {
                        id: id
                    },
    ...
    update: function (is_quit = 0) {
        ...
        if (form.valid()) {
            ...
            data.push({ name: "content_post", value: getValueTinymce("content_post") });
            $.ajax({
                url: laroute.route('discount-posts.update'),
                method: 'POST',
                data: data,
                success: function (res) {
                    if (res.error == false) {
